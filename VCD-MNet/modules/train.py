...
class RETrainer(BaseTrainer):
        ...
        with tqdm(total=self.train_num_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True, initial=self.step) as pbar:
            ...
            for epoch in range(1, self.args.num_epochs+1):
                ...
                for batch in self.train_data:
                    ...
                    (loss, logits), labels,seq, img, relation,head_pos,tail_pos = self._step(batch, mode="train")
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    ...
                    (loss, logits), labels,seq, img, relation,head_pos,tail_pos = self._step(batch, mode="dev")    # logits: batch, 3
                    total_loss += loss.detach().cpu().item()
    ...
    def _step(self, batch, mode="train"):
        if mode != "predict":
            ###dep 改加入valid, e1_mask, e2_mask, dep_type_matrix,b_use_valid_filter
            if self.args.use_prompt:
                if self.args.use_dep:
                    input_ids, token_type_ids, attention_mask, labels, images, aux_imgs, seq, img, relation, seq_input_ids, seq_token_type_ids, seq_attention_mask,\
                    valid, e1_mask, e2_mask, dep_type_matrix,b_use_valid_filter,head_pos, tail_pos = batch
                else:
                    valid, e1_mask, e2_mask, dep_type_matrix=None,None,None,None
            ...
            outputs = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                                 ...
                                 seq_attention_mask=seq_attention_mask,valid_ids=valid, e1_mask=e1_mask, e2_mask=e2_mask,
                                 dep_type_matrix=dep_type_matrix,dep_adj_matrix=dep_type_matrix)
            ###dep!!
